...
const selectedCatalog = ref<SupplierProduct | null>(null)
...
const purchaseFilters = reactive({ purchase_no: '', department_id: '', ordered_at: '', arrived_at: '', status: '' })
...
async function openSupplierDetail(id: number) {
  clearMessages()
  try { selectedSupplier.value = await getSupplier(id); supplierDetailOpen.value = true }
  catch (reason) { error.value = getErrorMessage(reason) }
}

async function openCatalogDetail(id: number) {
  clearMessages()
  try { selectedCatalog.value = await getSupplierProduct(id); catalogDetailOpen.value = true }
  catch (reason) { error.value = getErrorMessage(reason) }
}

function openSupplierEditor() {
  if (!selectedSupplier.value) return
  Object.assign(supplierEditForm, {
  ...
  supplierEditOpen.value = true
}

function openCatalogEditor() {
  if (!selectedCatalog.value) return
  Object.assign(catalogEditForm, {
    category_id: selectedCatalog.value.category_id || 0,
    name: selectedCatalog.value.name,
    unit_cost: selectedCatalog.value.unit_cost,
    shelf_life_days: selectedCatalog.value.shelf_life_days || 1,
    reason: '',
  })
...
async function submitSupplierEdit() {
  ...
  saving.value = true; clearMessages()
  try {
    selectedSupplier.value = await updateSupplier(selectedSupplier.value.id, {
      name: supplierEditForm.name,
      contact_name: supplierEditForm.contact_name || undefined,
      phone: supplierEditForm.phone || undefined,
      address: supplierEditForm.address || undefined,
      reason: supplierEditForm.reason || undefined,
    })
    supplierEditOpen.value = false; notice.value = '供应商资料修改成功'; await loadAll()
  } catch (reason) { error.value = getErrorMessage(reason) }
  finally { saving.value = false }
}
...
async function submitCatalogEdit() {
  ...
  saving.value = true; clearMessages()
  try {
    selectedCatalog.value = await updateSupplierProduct(selectedCatalog.value.id, {
      category_id: catalogEditForm.category_id,
      name: catalogEditForm.name,
      unit_cost: catalogEditForm.unit_cost,
      shelf_life_days: catalogEditForm.shelf_life_days,
      reason: catalogEditForm.reason || undefined,
    })
    catalogEditOpen.value = false; notice.value = '供应商商品修改成功'; await loadAll()
  } catch (reason) { error.value = getErrorMessage(reason) }
  finally { saving.value = false }
}
  ...
  <div>
    ...
    <section v-else class="panel table-panel">
      ...
      <div class="table-wrap"><table><thead><tr><th>编号 / 名称</th><th>联系人</th><th>电话</th><th>地址</th><th>状态</th><th></th></tr></thead><tbody>
        <tr v-for="item in suppliers" :key="item.id"><td><button class="text-button" @click="openSupplierDetail(item.id)"><strong>{{ item.name }}</strong></button><small class="block">{{ item.supplier_no }}</small></td><td>{{ item.contact_name || '—' }}</td><td>{{ item.phone || '—' }}</td><td>{{ item.address || '—' }}</td><td>{{ item.is_active ? '合作中' : '已停用' }}</td><td><button v-if="auth.isManager" class="text-button" @click="toggleSupplier(item)">{{ item.is_active ? '停用' : '启用' }}</button></td></tr>
      </tbody></table></div>
    </section>
...
.purchase-filters{display:flex;gap:9px;flex-wrap:wrap;padding:14px 24px;border-bottom:1px solid var(--line)}
</style>
